...
* A Javascript library. If WebAudio Visual Editor comes with some JS code, it's not designed to be used as a library but as a code-source generator.
* A digital audio workstation. In fact, WebAudio Visual Editor doesn't even produce sound by itself. Instead, it uses the browser's capabilities to generate sound. It makes it easy to produce sound making virtual devices and deploy them on websites.
...
* Produce code with **no dependency** : you can just copy and paste the code generated with WebAudio Visual Editor without worrying about including externals. The output code is vanilla Javascript and only uses code defined in the WebAudio API. It uses the _module_ pattern such as the code produced won't interfere with other Javascript code.
